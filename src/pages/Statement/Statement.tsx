...
const Statement = () => {
  ...
  const { fromDale, get, setValues, clearFromDale, values: prefillValues } = useFormPrefill('/statement');
  ...
  }, [account, period, zeroTurnover, daily, revaluation, trackField, completeDraft]);

  useEffect(() => {
    autoGenerateDone.current = false;
  }, [prefillValues]);

  useEffect(() => {
    if (!fromDale || !reference || autoGenerateDone.current || loading) return;

    const prefillPeriod = get('period');
    const prefillAccount = get('account');

    if (prefillPeriod && period !== prefillPeriod) return;
    if (prefillAccount) {
      const expectedAccount = reference.accountOptions.some(o => o.key === prefillAccount)
        ? prefillAccount
        : 'all';
      if (account !== expectedAccount) return;
    }

    autoGenerateDone.current = true;
    void handleGenerate();
  }, [fromDale, reference, period, account, get, handleGenerate, loading]);
